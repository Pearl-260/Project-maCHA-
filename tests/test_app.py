...
def test_reports_page_uses_database_records():
    app = create_app({"TESTING": True, "SQLALCHEMY_DATABASE_URI": "sqlite:///:memory:"})
    ...
    with app.app_context():
        ...
        Member(full_name="John Doe", email="john@example.com", phone="0722222222", group_name="Group A", join_date="2026-01-01", status="Active").save()
        Group(group_name="Group A", monthly_contribution=2000, meeting_day="Monday", max_members=10, status="Active").save()
        db.session.commit()
